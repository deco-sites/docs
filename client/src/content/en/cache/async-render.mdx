...
The client uses an `IntersectionObserver` to trigger the fetch when a section is within 200px of the viewport — so off-screen sections don't load until the user scrolls near them.
...
- If any loader uses `cache = "no-store"` or returns `null` from `cacheKey`, the section response is **not cached** at the CDN.
- If all loaders define a stable `cacheKey` and use `"stale-while-revalidate"` (or `{ maxAge }`), the section **is cached**.
