...
O script gera todos. Se for fazer na mão, copie dos templates em [`@decocms/start/.agents/skills/deco-to-tanstack-migration/templates/`](https://github.com/decocms/deco-start/tree/main/.agents/skills).
...
As lojas de referência têm tudo isso como scripts compostos no `package.json`.
...
Veja [`use-client-migration.md`](/Users/fernandofrizzatti/development/workspace/decocms/casaevideo-storefront/docs/use-client-migration.md) do Casa&Vídeo para um mapa exaustivo de island por island de uma loja real.
...
Scripts que mutam o `<head>` (Google Tag Manager, Adobe Launch, Hotjar) frequentemente causam mismatch de hidratação em React. A correção é injetar via resposta do worker, depois do React renderizar, em vez de em JSX. Veja [`cls-pending-issues.md`](/Users/fernandofrizzatti/development/workspace/decocms/casaevideo-storefront/docs/cls-pending-issues.md) do Casa&Vídeo.
...
- **Pode `loaders.gen.ts`** — use `--decofile-dir .deco/blocks` para que só apareçam loaders que o CMS realmente referencia. Bagaggio usa; Casa e Le biscuit ainda não.
