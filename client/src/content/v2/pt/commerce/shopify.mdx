...
`responseHeaders` é onde `getCart` seta o cookie do carrinho. Esquecer significa cliente nunca persiste carrinho — adiciona item, refresh, vazio. Diferente da VTEX, o Shopify **não tem middleware de cart**: você passa `responseHeaders` para `getCart` sempre que quiser garantir/criar o cookie.
...
```ts
import type { Product, ProductDetailsPage, Cart } from "@decocms/apps-commerce/types";
```

Os loaders do Shopify já devolvem os tipos schema.org compartilhados de `@decocms/apps-commerce/types`.
...
Variants Shopify são produtos separados na API mas a maioria das UI quer apresentar agrupados. Use `transformShopifyProduct` em `@decocms/apps-shopify/utils/transform.ts`.
