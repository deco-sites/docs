...
**Não tente chunk manual de `@decocms/blocks` ou `@decocms/apps-*`** no `vite.config.ts`. O Vite plugin cuida do fast path do JSON, e esses pacotes têm re-exports circulares que crasham se forem divididos em chunks.
</Callout>
...
2. `loadCmsPage` chama `resolveDecoPage` de `@decocms/blocks/cms`, que:
