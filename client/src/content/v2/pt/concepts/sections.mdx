...
`@decocms/start` aplica algumas convenções automáticas em sections no momento do registro. Veja [`applySectionConventions`](/Users/fernandofrizzatti/development/workspace/decocms/deco-start/src/cms/applySectionConventions.ts):
...
Você também pode registrar sections explicitamente via `registerSectionLoaders` de `@decocms/start/cms` se precisar de controle fino. Veja [`casaevideo-storefront/src/setup.ts`](/Users/fernandofrizzatti/development/workspace/decocms/casaevideo-storefront/src/setup.ts) para um exemplo real.
