...
O gerador de TS → JSON Schema entende anotações extras de `@decocms/blocks/types/widgets`:
...
`@decocms/blocks` aplica algumas convenções automáticas em sections no momento do registro. Veja `applySectionConventions`:
...
Você também pode registrar sections explicitamente via `registerSectionLoaders` de `@decocms/blocks/cms` se precisar de controle fino sobre a ordem ou quiser filtrar o glob.
