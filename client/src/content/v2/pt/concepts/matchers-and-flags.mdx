...
`@decocms/blocks/matchers/builtins` traz matchers prontos:
...
`@decocms/blocks/matchers/posthog` conecta com feature flags do PostHog:
...
`@decocms/blocks/sdk/abTesting` exporta `withABTesting`, um wrapper de worker que roda lógica de A/B antes do framework sequer ver o request — útil para comparar versões inteiras do site.
