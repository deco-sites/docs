...
`createSiteSetup` é a função única que faz bootstrap de uma loja v2. Registra suas sections, blocks, matchers e integrações de plataforma em um lugar. Toda loja de referência tem um `src/setup.ts` que chama ela.
...
`applySectionConventions` e `registerLayoutSections` são quase sempre chamados. O resto depende do que seu site usa. Veja o [`setup.ts` do Casa&Vídeo](/Users/fernandofrizzatti/development/workspace/decocms/casaevideo-storefront/src/setup.ts) para o pacote completo, ou o [do Bagaggio](/Users/fernandofrizzatti/development/workspace/decocms/baggagio-tanstack/src/setup.ts) para o mínimo.
