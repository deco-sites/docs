...
`@decocms/blocks/middleware` exporta funções que rodam contra o request/response.
...
`@decocms/blocks/matchers/builtins` provê matchers prontos para A/B, segmentação e flags:
...
`@decocms/blocks/matchers/posthog` integra com flags do PostHog:
...
Por padrão, `matchPosthog` lê o ID `distinct` do PostHog do cookie. Sem cookie, gera um por request. Para sticky entre sessões, garanta que o cookie do PostHog é setado pelo seu snippet PostHog client-side ou via `setPosthogCookie` (`@decocms/blocks/matchers/posthog`).
