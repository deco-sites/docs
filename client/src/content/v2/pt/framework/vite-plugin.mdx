...
Módulos que só existem no servidor (internos do registro `@decocms/blocks/cms`, `node:async_hooks`, `node:fs`) viram stubs no build do client. Sem isso, o bundle quebraria no import.
...
Em modo dev, o plugin sobe o daemon (file watcher, tunnel JWT-autenticado para o admin) para que `admin.deco.cx` consiga falar com seu server local. Veja `@decocms/tanstack/daemon` para o motor.

### 5. Dicas de chunk

O plugin orienta o Vite a **não** dividir certos módulos em chunks separados:

- `@decocms/blocks` e `@decocms/apps-*` têm re-exports circulares entre submódulos. Dividir em chunks causa crashes na ordem de load.
...
**Não configure `manualChunks` manualmente para `@decocms/blocks` ou `@decocms/apps-*`.**
...
| `@decocms/blocks/cms` (registry interno) | ✓ | stub |
...
Você está dividindo `@decocms/blocks` ou `@decocms/apps-*` em chunks. Remova seu override de `manualChunks`.
