...
Essa é a baseline canônica — toda loja de referência usa esse formato com diferenças cosméticas. Veja [`vite.config.ts` em lebiscuit-tanstack`](/Users/fernandofrizzatti/development/workspace/decocms/lebiscuit-tanstack/vite.config.ts) para um real.
...
Importa: o `blocks.gen.json` da Casa&Vídeo tem 11 MB. Sem fast-path, cold starts disparam.
...
Em modo dev, o plugin sobe o daemon (file watcher, tunnel JWT-autenticado para o admin) para que `admin.deco.cx` consiga falar com seu server local. Veja [`@decocms/start/daemon`](/Users/fernandofrizzatti/development/workspace/decocms/deco-start/src/daemon/) para o motor.
