...
`/deco/invoke` é o endpoint que o browser usa para chamar loaders e actions server-side. `@decocms/blocks/sdk/invoke` provê o cliente — bruto e tipado — para que seu código React converse com ele sem montar URLs na mão.
...
`scripts/generate-invoke.ts` em `@decocms/blocks` lê seu `loaders.gen.ts` e emite um `invoke.gen.ts` com cliente tipado:
...
```ts
import { createInvoke } from "@decocms/blocks/sdk/invoke";
...
```ts
import { setInvokeLoaders } from "@decocms/blocks/sdk/invoke";
