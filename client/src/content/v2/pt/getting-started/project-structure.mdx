...
Uma loja v2 tem uma superfície deliberadamente pequena. A maioria dos arquivos é gerada. Esta página percorre o layout que você vai ver depois de clonar uma loja de referência.
...
Toda loja de referência — [casaevideo](/Users/fernandofrizzatti/development/workspace/decocms/casaevideo-storefront), [baggagio](/Users/fernandofrizzatti/development/workspace/decocms/baggagio-tanstack), [lebiscuit](/Users/fernandofrizzatti/development/workspace/decocms/lebiscuit-tanstack) — segue essa forma.
...
- **[`src/routes/index.tsx`](/Users/fernandofrizzatti/development/workspace/decocms/lebiscuit-tanstack/src/routes/index.tsx)** — homepage; usa `cmsHomeRouteConfig`.
- **[`src/routes/$.tsx`](/Users/fernandofrizzatti/development/workspace/decocms/lebiscuit-tanstack/src/routes/$.tsx)** — catch-all para URLs guiadas pelo CMS; usa `cmsRouteConfig`.
...
**`blocks.gen.json` pode ser grande** (o da Casa&Vídeo tem 11,6 MB). O Vite plugin troca o re-export `.ts` por um `JSON.parse` rápido do `.json` em load time. Não tente chunk manual de `@decocms/start` ou `@decocms/apps` — re-exports circulares quebram em load.
</Callout>
