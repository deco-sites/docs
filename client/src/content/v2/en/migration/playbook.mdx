...
The script generates these. If you're doing this by hand, copy from the templates in [`@decocms/start/.agents/skills/deco-to-tanstack-migration/templates/`](https://github.com/decocms/deco-start/tree/main/.agents/skills).

## Phase 3 — Transform

This is the bulk-rewrite phase:

- **Imports** — `preact/*` → `react`, `@preact/signals` → `@tanstack/store`, `$fresh/*` → `@tanstack/react-start`, `@deco/deco/*` → `@decocms/start/*` and `@decocms/apps/*`.
...
The reference sites have all of these as composite scripts in `package.json`.
...
See [Casa&Video's `use-client-migration.md`](/Users/fernandofrizzatti/development/workspace/decocms/casaevideo-storefront/docs/use-client-migration.md) for an exhaustive island-by-island map from a real site.
...
- **`inlineScript`** from `@decocms/start/sdk/useScript` for static script content.
...
Scripts that mutate `<head>` (Google Tag Manager, Adobe Launch, Hotjar) often cause hydration mismatches in React. The fix is to inject them in the worker response, after React has rendered, instead of in JSX. See [Casa&Video's `cls-pending-issues.md`](/Users/fernandofrizzatti/development/workspace/decocms/casaevideo-storefront/docs/cls-pending-issues.md).
...
- **Prune `loaders.gen.ts`** — use `--decofile-dir .deco/blocks` so only loaders the CMS actually references appear in the registry. Bagaggio uses this; Casa&Video and Le biscuit don't yet.
