...
The full registration logic lives in [`vtex/commerceLoaders.ts`](/Users/fernandofrizzatti/development/workspace/decocms/apps-start/vtex/commerceLoaders.ts).

## Caching layer

Every inline loader is wrapped in `createCachedLoader` from `@decocms/start/sdk/cachedLoader`:
...
A real-world enrichment from Casa&Video:
...
[`vtex/inline-loaders/minicart.ts`](/Users/fernandofrizzatti/development/workspace/decocms/apps-start/vtex/inline-loaders/minicart.ts) is special:
...
By default, every inline loader appears in your generated `loaders.gen.ts`. For sites with thousands of unused loaders, this bloats the bundle. Bagaggio uses the `--decofile-dir` flag to prune:
