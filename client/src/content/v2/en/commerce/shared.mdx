...
<Callout type="info">
**TODOs in the type file.** [`commerce/types/commerce.ts`](/Users/fernandofrizzatti/development/workspace/decocms/apps-start/commerce/types/commerce.ts) has open TODOs about JSON Schema generator support for recursive types. Some navigation types are marked `@deprecated`. Pin to current types and watch the changelog when upgrading.
</Callout>
...
```tsx
import { Image, registerImageCdnDomain } from "@decocms/apps/commerce/components/Image";
```
...
```ts
import { registerImageCdnDomain } from "@decocms/apps/commerce/components/Image";
...
```tsx
import { Picture, Source } from "@decocms/apps/commerce/components/Picture";
...
```tsx
import { ProductJsonLd, BreadcrumbJsonLd, PLPJsonLd } from "@decocms/apps/commerce/components/JsonLd";
