...
O cliente usa um `IntersectionObserver` para disparar a busca quando uma seção está dentro de 200px da viewport — seções fora da tela não carregam até o usuário rolar a página.
...
- Se qualquer loader usa `cache = "no-store"` ou retorna `null` do `cacheKey`, a resposta da seção **não é cacheada** na CDN.
- Se todos os loaders definem um `cacheKey` estável e usam `"stale-while-revalidate"` (ou `{ maxAge }`), a seção **é cacheada**.
