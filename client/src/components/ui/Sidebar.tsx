            ...
            name={
              node.id === "getting-started"
                ...
                : node.id === "cms-capabilities"
                  ? "LayoutDashboard"
                  : node.id === "developing-guide"
                    ? "Code"
                    : node.id === "concepts"
                      ? "Lightbulb"
                      : node.id === "developing-capabilities"
                        ? "Wrench"
                        : node.id === "decopilot"
                          ? "Bot"
                          : node.id === "performance"
...
export default function Sidebar({ tree, locale, translations }: SidebarProps) {
  ...
  useEffect(() => {
    ...
    // Initialize tree state - default to expanded (with archive collapsed by default)
    tree.forEach((node) => {
      if (node.type === "folder") {
        const saved = savedState[node.id];
        // Keep archive collapsed by default
        const collapsedByDefault = new Set<string>(["archive"]);
    ...
    <div className="flex flex-col h-screen bg-app-background border-r border-border w-[19rem] lg:w-[19rem] w-full max-w-[19rem]">
      ...
      <div className="px-4 lg:px-8 py-4 border-t border-border shrink-0">
        <div className="space-y-2">
          ...
          <a
            href="https://www.decocms.com/blog"
            target="_blank"
